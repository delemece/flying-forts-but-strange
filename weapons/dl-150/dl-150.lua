Scale = 1
SelectionWidth = 49.0
SelectionHeight = 40
SelectionOffset = { 0, -41 }
RecessionBox =
{
	Size = { 200, 25 },
	Offset = { -300, -70 },
}

WeaponMass = 110.0
HitPoints = 200.0
EnergyProductionRate = 0.0
MetalProductionRate = 0.0
EnergyStorageCapacity = 0.0
MetalStorageCapacity = 0.0
MinWindEfficiency = 1
MaxWindHeight = 0
MaxRotationalSpeed = 0
DeviceSplashDamage = 150
DeviceSplashDamageMaxRadius = 200
DeviceSplashDamageDelay = 0.2
IncendiaryRadius = 120
IncendiaryRadiusHeated = 150
StructureSplashDamage = 150
StructureSplashDamageMaxRadius = 150

FireEffect = path .. "/effects/fire_150.lua"
ConstructEffect = "effects/device_construct.lua"
CompleteEffect = "effects/device_complete.lua"
DestroyEffect = "effects/cannon_explode.lua"
DestroyUnderwaterEffect = "mods/dlc2/effects/device_explode_submerged_large.lua"
ShellEffect = "effects/shell_eject_cannon.lua"
ReloadEffectOffset = 0
RetriggerFireEffect = true
Projectile = "dl150"
--Projectile = "howitzer"
BarrelLength = 100.0 
MinFireClearance = 500
FireClearanceOffsetInner = 20
FireClearanceOffsetOuter = 40
ReloadTime = 2
MinFireSpeed = 10000.0
MaxFireSpeed = 10000.1
MinFireRadius = 600.0
MaxFireRadius = 1200.0
MinFireAngle = -5
MaxFireAngle = 80
KickbackMean = 15
KickbackStdDev = 3
MouseSensitivityFactor = 0.5
PanDuration = 0
FireStdDev = 0.01
FireStdDevAuto = 0.03
Recoil = 20000
EnergyFireCost = 800.0
MetalFireCost = 5
ShowFireAngle = true
RoundsEachBurst = 100
RoundPeriod = 0.3
--UniformSpray = true
ReloadFramePeriod = (ReloadTime + RoundsEachBurst*RoundPeriod)/12
DoorCloseDelay = 1
AutofireCloseDoorTicks = DoorCloseDelay*25

BarrelRecoilLimit = -0.2
BarrelRecoilSpeed = -3
BarrelReturnForce = 2

HoldToFire = true
CanUpdateAimWhileFiring = true

CanOverheat = true
HeatPeriod = 2.5
CoolPeriod = 40
CoolPeriodOverheated = 22


TargetIcon =
{
	Persistent = true,
	Texture = "ui/textures/mouse_target.tga",
	Width = 32,
	Height = 32,
}

dofile("effects/device_smoke.lua")
SmokeEmitter = StandardDeviceSmokeEmitter

Sprites =
{
	{
		Name = "dl-150-base",
		States =
		{
			Normal = { Frames = { { texture = path .. "/weapons/dl-150/base.png" }, mipmap = true, }, },
			Idle = Normal,
		},
	},
	{
		Name = "dl-150-head",
		States =
		{
			Normal = { Frames = { { texture = path .. "/weapons/dl-150/head.png" }, mipmap = true, }, },
			Idle = Normal,
		},
	},
	{
		Name = "dl-150-barrel",
		States =
		{
			Normal = { Frames = { { texture = path .. "/weapons/dl-150/barrel.png" }, mipmap = true, }, },
			Idle = Normal,
		},
	},
}

--[[NodeEffects =
{
	{
		NodeName = "Hardpoint0",
		EffectPath = "effects/weapon_overheated.lua",
		Automatic = false,
	},
}]]

Root =
{
	Name = "base",
	Angle = 0,
	Pivot = { 0.05, -0.5 },
	PivotOffset = { 0, 0 },
	Sprite = "dl-150-base",
	UserData = 0,
	
	ChildrenBehind =
	{
		{
			Name = "Head",
			Angle = 0,
			Pivot = { 0.0, -0.17 },
			PivotOffset = { 0.0, 0.0 },
			Sprite = "dl-150-head",
			UserData = 0,

			ChildrenBehind =
			{
				{
					Name = "Barrel",
					Angle = 0,
					Pivot = { 0.25, 0.0 },
					PivotOffset = { 0.0, 0.0 },
					Sprite = "dl-150-barrel",
					UserData = 0,

					ChildrenBehind =
			
					{
						{
							Name = "Hardpoint0",
							Angle = 90,
							Pivot = { 0.5, -0.1 },
							PivotOffset = { 0, 0 },
						},
						{
							Name = "Chamber",
							Angle = 0,
							Pivot = { -0.5, 0.0 },
							PivotOffset = { 0, 0 },
						},
					},
				},
			},
		},
	},
}
